    ...
    <div class="container flex items-center max-w-8xl mx-auto px-4 lg:px-8">
        ...
        <div class="flex flex-1 justify-end items-center text-right md:pl-10 text-gray-800">
            <navigation api-key="{{ $page->docsearchApiKey }}" index="{{ $page->docsearchIndexName }}">
                @if($page->isOnLaravel())
                    <x-menu-link href="{{ $page->route('docs.core', ['version' => $page->getLatestVersion('core')]) }}" title="LdapRecord Documentation Link">
                        {{ $page->isOnLaravel() ? 'Core Docs' : 'Docs' }}
                    </x-menu-link>

                    <a href="{{ $page->route('docs.laravel.source') }}" class="hidden text-gray-800 hover:text-purple-700 sm:inline">
                ...
                @else
                    @if($page->isHomePage())
                        <x-menu-link href="{{ $page->route('docs.core', ['version' => $page->getLatestVersion('core')]) }}" title="LdapRecord-Laravel Documentation Link">
                            Docs
                        </x-menu-link>
                    @endif
                    ...
                    <x-github-link url="{{ $page->route('docs.core.source') }}" class="hidden md:inline text-gray-800 hover:text-purple-700"></x-github-link>
                @endif
            </navigation>
        </div>
    </div>
